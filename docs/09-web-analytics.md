...
对应：REQ-WEB-001 ~ REQ-WEB-008、REQ-EXPORT-001、REQ-TECH-003。
...
- UI：清晰的岗位情报台，不做营销落地页
- 图表：shadcn/ui Charts（Recharts）与/或 Apache ECharts（echarts-for-react）；筛选与图表联动（REQ-WEB-007）
- 动效：关键过渡与交互反馈，服务信息理解（REQ-WEB-008）
...
|------|------|
| `/` | 概览：最近采集、岗位总数、快捷入口 |
| `/jobs` | 岗位列表：城市/平台/关键词/薪资筛选；支持导出 Excel |
| `/jobs/[id]` | 岗位详情：JD、福利、休息、公司链接 |
| `/tasks` | 采集任务：创建（城市+关键词+平台）、状态 |
| `/analytics` | 分析看板（图表 + 筛选联动） |
...
### 3.3 图表与交互（REQ-WEB-007 / REQ-WEB-008）

- 分析页筛选变更后图表同步更新
- 必要的过渡与悬停反馈；不做纯装饰性动效堆砌

### 3.4 可选增强（有余力再做，需改需求状态）

- 福利词频、双休占比、公司重复发岗 Top N

## 4. Excel 导出（REQ-EXPORT-001）

- Web 与/或 CLI 可将当前筛选范围内的岗位导出为 Excel
- 字段覆盖采集核心列（岗位名、城市、薪资、公司、平台、福利、休息、JD 等）
